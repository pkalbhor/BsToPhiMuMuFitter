...
from BsToPhiMuMuFitter.varCollection import Bmass, CosThetaK, CosThetaL, Phi, Bdt
...
pdfP = "1+p1*Phi+p2*pow(Phi,2)+p3*pow(Phi,3)+p4*pow(Phi,4)+p5*pow(Phi,5)+p6*pow(Phi,6)"
xTerm = "\
...
f_effiSigA_format['DEFAULT'] = ["l{0}[-10,10]".format(i) for i in range(1, 6 + 1)] \
    ...
    + ["expr::effi_sigA('effi_norm*({pdfL})*({pdfK})*({pdfP})*(1+hasXTerm*({xTerm}))', {args})".format(
        ...
        pdfP=pdfP,
        xTerm=xTerm,
        args="{CosThetaL,CosThetaK, Phi, hasXTerm,effi_norm," + ','.join(["l{0}".format(i) for i in range(1, 7)] + ["k{0}".format(i) for i in range(1, 7)] + ["p{0}".format(i) for i in range(1, 7)] + ["x{0}".format(i) for i in range(16)]) + "}")]
...
setupBuildEffiSigA = {
    'objName': "effi_sigA",
    'varNames': ["CosThetaK", "CosThetaL", "Phi"],
    'factoryCmd': [
    ]
}
...
def buildSigA(self):
    ...
    if f_sigA == None:
        ...
        wspace.factory("Fl[0.30,0.00001,0.9]")
        wspace.factory("S3[-0.09,-0.3,0.3]")
        wspace.factory("S4[-0.1,-0.8,0.8]")
        wspace.factory("A5[0.1,-0.3,0.3]")
        wspace.factory("A6[-0.2,-0.5,0.5]")
        wspace.factory("S7[0.1,-0.5,0.5]")
        wspace.factory("A8[-0.1,-0.5,0.5]")
        wspace.factory("A9[0.03,-0.5,0.5]")
        ...
        wspace.factory("EXPR::f_sigA_original('0.08952465549*(0.75*(1-Fl)*(1.-pow(CosThetaK, 2)) + Fl*pow(CosThetaK, 2) + 0.25*(1.-Fl)*(2*pow(CosThetaK, 2)-1.0-2.0*pow(CosThetaK, 2)*pow(CosThetaL, 2) + pow(CosThetaK, 2)) + Fl*(pow(CosThetaK, 2)-2.*pow(CosThetaK,2)*pow(CosThetaL, 2)) + S3*(1.-pow(CosThetaK, 2))*(1.-pow(CosThetaL, 2))*(2.*TMath::Cos(2.*Phi)) + S4*(2.*pow(CosThetaK, 2)-1.)*(2.*pow(CosThetaL,2)-1.0)*(TMath::Cos(Phi)) + A5*(2.*pow(CosThetaK, 2)-1.)*(TMath::Sqrt(1.-pow(CosThetaL, 2)))*TMath::Cos(Phi) + A6*(1.-pow(CosThetaK, 2))*CosThetaL + S7*(2.*pow(CosThetaK, 2)-1.)*TMath::Sqrt(1.- pow(CosThetaL, 2))*TMath::Sin(Phi) + A8*(2.*pow(CosThetaK, 2)-1.)*(2.*pow(CosThetaL, 2)-1.) + A9*(1.-pow(CosThetaK, 2))*(1.-pow(CosThetaL, 2))*TMath::Sin(2*Phi))', {CosThetaK, CosThetaL, Phi, Fl, S3, S4, A5, A6, S7, A8, A9})")
...
f_analyticBkgCombA_format['belowJpsiA'] = [
    ...
    "bkgCombK_c2[0,50]",
    "bkgCombK_c3[-3,0]",
    "EXPR::f_bkgCombA('({pdfL})*({pdfK})*({pdfP})', {args})".format(
        ...
        pdfP="",
        args="{CosThetaL, CosThetaK, Phi, bkgCombL_c1, bkgCombL_c2, bkgCombL_c3, bkgCombL_c4, bkgCombL_c5, bkgCombK_c1, bkgCombK_c2, bkgCombK_c3}")
]
...
CFG_WspaceReader.update({
    'obj': OrderedDict([])  # Empty by default loads all Functions and Pdfs
})
